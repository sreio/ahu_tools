...
- 支持背景色。
- 支持上传一张背景图片。
- 支持在未上传背景图时选择“主题背景”，由 Canvas 生成好看的随机主题样式。
- 背景优先级：背景图 > 主题背景 > 背景色。
- 主题背景内置风格：
  - 极光渐变。
  - 玻璃拟态。
  ...
  - 杂志封面。
  - 自然柔光。
- 支持“随机一次”，重新生成当前主题的 seed 和视觉参数。
- 支持“锁定样式”，锁定后生成预览不会自动改变主题背景。
- 主题背景不依赖网络图片，不读取外部资源。
- 背景图片填充方式：
  - 拉伸：铺满画布，可能变形。
  - 等比覆盖：覆盖整个画布，可能裁掉边缘。
...
- 背景：背景来源、背景色、主题风格、随机一次、锁定样式、背景图片上传、填充方式。
...
- `backgroundColor`。
- `backgroundMode`：`color`、`theme`、`image`。
- `themeType`。
- `themeSeed`。
- `themeLocked`。
...
- 选择主题背景并点击“随机一次”，预览背景发生变化。
- 打开“锁定样式”后，重复生成预览保持同一主题背景。
- 上传背景图后，优先显示背景图，不显示主题背景。
